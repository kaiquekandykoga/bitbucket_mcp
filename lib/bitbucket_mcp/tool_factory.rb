...
module BitbucketMcp
  ...
  #
  # Every Bitbucket tool maps 1:1 to a {BitbucketMcp::Client} method of the same
  # name, so a single handler forwards the validated arguments straight through.
  # Parameter defaults therefore live in one place: the client method signature.
  module ToolFactory
    module_function
    ...
    # @param destructive [Boolean] true for tools that delete or irreversibly change data.
    ...
    # result as an MCP tool response. Known Bitbucket failures become a clean
    # tool error rather than a transport-level error.
    def invoke(name, args)
      result = Client.new.public_send(name, **args)
      text = result.is_a?(String) ? result : JSON.pretty_generate(result)
      MCP::Tool::Response.new([{ type: "text", text: text }])
    rescue BitbucketMcp::Error => e
